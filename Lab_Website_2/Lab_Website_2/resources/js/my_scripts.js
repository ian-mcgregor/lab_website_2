...
function loadStatsPage(){
	...
	var table = document.getElementById("stats_table"); //retrieve stats table element
	var rowCt;
	var colCt;
	var home;
	var away;
	...
	for(rowCt = 2; rowCt < table.rows.length; rowCt++){
		//Outer iterates through each row
		// for(colCt = 2; colCt < table.rows[rowCt].cells.length; colCt++){
		// //Inner iterates through columns
		colCt = 2;
		home = table.rows[rowCt].cells[colCt].innerHTML;//Read in a cells current value
		away = table.rows[rowCt].cells[colCt + 1].innerHTML;
		home = parseInt(home);
		away = parseInt(away);
		if(home > away){
			table.rows[rowCt].cells[colCt + 2].innerHTML = "CU Boulder";
			//id = wins
			wins++;
		}
		else{
			table.rows[rowCt].cells[colCt + 2].innerHTML = table.rows[rowCt].cells[colCt - 1].innerHTML;
			//id = losses
			losses++;
